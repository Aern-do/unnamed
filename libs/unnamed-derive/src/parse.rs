...
use quote::quote;
use syn::{Data, DataStruct, DeriveInput, Error, Expr, Field, Generics, Ident};

pub(crate) fn expand(derive_input: DeriveInput) -> syn::Result<TokenStream> {
    match derive_input.data {
        Data::Struct(data_struct) => {
            Ok(expand_data_struct(derive_input.ident, derive_input.generics, data_struct)?)
        }
...
fn expand_data_struct(
    ...
    let fields = data_struct.fields.iter().map(
        |Field { ident, attrs, ty, .. }| -> syn::Result<TokenStream> {
    ...
    let lt = if !params.is_empty() { Some(quote!('source)) } else { None };
